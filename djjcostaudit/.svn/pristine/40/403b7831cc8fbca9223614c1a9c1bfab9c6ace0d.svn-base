    ...
    $('#serviceProjectStatistics').datagrid({
        ...
        columns: [
            [{
                ...
                formatter:function(value,rowData,index){
                    if(value!=null){
                        var str="<a href='javascript:void(0)' title='跳转合同' onclick='editInfoL(\"" + rowData.contractId + "\")' style=\"color:#549de3;\">"+addQianFenFu(String(value))+"</a>";
                        return str;
                        // return addQianFenFu(String(value))
                    }else{
                        return ""
...
//导出
function erExportDate1(code){
    // var name=$("#name").val();
    var selections = $('#serviceProjectStatistics').datagrid('getSelections');
    var ids = "";
    if (selections.length != 0) {
        for(var i = 0;i<selections.length;i++){
            ids+=selections[i].id+",";
    ...
    window.location.href = "/yaohaoWinbid/erExportYaohaoWinbid?&ids="+ids+"&code="+code;
}