    ...
    <form action="" id="formId">
    	...
    	<table class="table_edit" id="table" style="margin-bottom: 10px;" cellspacing="0" cellpadding="0" >
                ...
                <tr>
                    ...
                    <td >
                        <select disabled id="type" name="type" class="custom-select" onchange="typeChange();" style="width:50%">
                ...
                <tr id="remarkBz">
                	...
                	<td>
                    	<textarea readonly="readonly" id="remark" name="remark" rows="2">${obj.remark}</textarea>
                	</td>
            	</tr>
            	</c:if>
                <tr>
                    ...
                    <td>
                        <input disabled class="Wdate search_text_form"  type="text" value="<fmt:formatDate value="${obj.leaveStartTime}" pattern="yyyy-MM-dd"/> - <fmt:formatDate value="${obj.leaveEndTime}" pattern="yyyy-MM-dd"/>" name="leaveTime" id="leaveTime" style="width:50%;margin-right:20px">
                        <input disabled id="am"  <c:if test="${obj.morAftAll eq '上午' }">checked</c:if>    name="morAftAll" value="上午" type="radio"><label for="am">上午</label>&nbsp;&nbsp;
                        <input disabled id="pm"   <c:if test="${obj.morAftAll eq '下午' }">checked</c:if>    name="morAftAll" value="下午" type="radio"><label for="pm">下午</label>&nbsp;&nbsp;
                        <input disabled id="all"  <c:if test="${obj.morAftAll eq '全天' }">checked</c:if>  value="全天" name="morAftAll" type="radio"><label for="all">全天</label>&nbsp;&nbsp;
                    </td>
                ...
                <tr>
                    <td class="bgcolor"><strong>*</strong>销假原因：</td>
                    <td>
                        <textarea id="opinion" name="opinion"  rows="2"></textarea>
                    </td>
                </tr>
                <c:if test="${obj.status eq '结束'}">
                    <tr>
                        <td class="bgcolor"><strong>&nbsp;</strong>销假传阅对象：</td>
                        <td style="text-align: left">
                            <input type="text"  readonly="readonly" name="relatedId" value="${obj.relatedName }" placeholder="请选择传阅对象"  style="width:80%;margin-top:5px;">
                        </td>
                    </tr>
                </c:if>
                ...
                <tr style="height:40px">
                    <td colspan="2" style="text-align: center">
                        <input type="button" class="btn btn-primary" id="save" value="确认销假">&nbsp;&nbsp;
                        <input type="button" class="btn btn-success" onclick="top.closeWindow('请假申请销假')" value="取消">
                    </td>
                </tr>
                </tbody>
            </table>
...
<script>
	...
	$(function(){
		...
		$("#save").click(function(){
            ...
            if(opinion==""){
                $.messager.alert({title:'温馨提示', msg:'请填写销假原因！', icon: 'error', top:100});
                return;
            }
			$.ajax({
				  url: "/workProcessApply/repeal",
				  dataType: "json",
				  type:"post",
				  async: false,
