...
public class YaohaoOrderServiceImpl implements YaohaoOrderService {
	...
	private final Logger LOG = LogManager.getLogger(YaohaoOrderServiceImpl.class);
	...
	public int registerOrder(String winbid ,YaohaoOrder yaohaoOrder) throws Exception {
		...
		if(null!=list && list.size()>0){
			...
			for(YaohaoWinbid bid:list){
				mapTem.put("nameEq", bid.getName());
				List<YaohaoWinbid> winbidList=yaohaoWinbidMapper.selectListByMap(mapTem);
				mapTem.clear();
			...
			if(!flag){
				...
				int temp=yaohaoOrderMapper.insertSelective(yaohaoOrder);
				if(temp<=0){
					LOG.error("插入异常！");
				}
				for(YaohaoWinbid bid:list){
					...
					yaohaoWinbid.setRemark(bid.getRemark());
					int p=yaohaoWinbidMapper.insertSelective(yaohaoWinbid);
					if(p<=0){
						LOG.error("插入异常！");
					}
					//候选人表加入摇号单
					...
					int n=yaohaoCandidateMapper.updateByPrimaryKeySelective(can);
					if(n<=0){
						LOG.error("更改异常！");
					}
				}
				...
				if(null!=crList && crList.size()>0){
					...
					for(YaohaoCandidateRemove cr:crList){
						...
						cd.setRemoveFlag("1");
						int n=yaohaoCandidateMapper.updateByPrimaryKeySelective(cd);
						if(n<=0){
							LOG.error("更改异常！");
						}
	...
	private void getAutoCode(YaohaoOrder yaohaoOrder) {
		...
		int temp=yaohaoAutocodeMapper.updateByPrimaryKeySelective(code);
		if(temp<=0){
			LOG.error("更新异常！");
		}
	}
	...
	private void buildCandidate(YaohaoOrder yaohaoOrder,Map<String, Object> map, int l) {
		...
		if("第二档".equals(yaohaoOrder.getYaohaoGrade())){
			...
			for(CostEnterprise c: eList){
				...
				int n=yaohaoCandidateMapper.insertSelective(canNew);
				if(n<=0){
					LOG.error("更新异常！");
				}
			}
	...
	public int updRegisterOrder(String winbid, YaohaoOrder yaohaoOrder)throws Exception {
		...
		if(null!=list && list.size()>0){
			...
			for(YaohaoWinbid bid:list){
				...
				criteria.andIdNotEqualTo(bid.getId());
			...
			if(!flag){
				...
				int u=yaohaoOrderMapper.updateByPrimaryKeySelective(yaohaoOrder);
				if(u<=0){
					LOG.error("更新异常！");
				}
				int d=yaohaoWinbidMapper.deleteByOrderCode(yaohaoOrder.getCode());
				if(d<=0){
					LOG.error("删除异常！");
				}
				...
				for(YaohaoWinbid bid:list){
					...
					yaohaoWinbid.setCreater(currentUser.getName());
					yaohaoWinbid.setRemark(bid.getRemark());
					int i=yaohaoWinbidMapper.insertSelective(yaohaoWinbid);
					if(i<=0){
						LOG.error("插入异常！");
					}
					//剔除掉候选名单bid_flag
				...
				for(YaohaoCandidate c:canList){
					if(!eCodeList.contains(c.getEnterpriseCode())){
						c.setBidFlag("");
						int upd=yaohaoCandidateMapper.updateByPrimaryKeySelective(c);
						if(upd<=0){
							LOG.error("更新异常！");
						}
					}
