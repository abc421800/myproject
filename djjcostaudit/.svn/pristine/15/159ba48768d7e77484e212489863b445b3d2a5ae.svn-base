...
$(function() {
    ...
    $('#containContract').datagrid({
        url: '/costContract/list?enterpriseCode=${param.enterpriseCode}',
        loadMsg: '数据加载中,请稍候...',
        nowrap: false,
        rownumbers: true,
        height: 300,
        ...
        onLoadSuccess: function (data) {
            if (data.total == 0) {
                $('#containContract').datagrid('loadData', {total: 1, rows: [{code: "未找到相关信息！"}]});
            }
        },
    ...
    $("#saveContainCon").click(function(){
   	 var rows = $('#containContract').datagrid('getSelections');
	   	...
	   	$.ajax({
	         url: '/yaohaoWinbid/containContract',
	         type: 'POST',
	         data: {'winId': winId,'contractId':rows[0].id},
	         traditional: true,
	         success: function (result){
	             if (result.status==200) {
	            	 window.parent.$('#winningRecord').datagrid('reload');
