...
<div data-options="region:'center',border:0" class="wrap_center">
    <form action="" id="formId">
    	...
    	<table class="table_edit" style="margin-bottom: 10px;" id="table" cellspacing="0" cellpadding="0" >
                <tbody>
                ...
                <tr>
                    ...
                    <td >
                    	<select id="type" name="type" class="custom-select" style="width:50%">
	                    	<c:forTokens var="t" items="年休假,探亲假,事假,生育假,计生假,病假,婚假,丧假,离岗学习,出差,其他" delims=",">
	                            <option <c:if test="${obj.type eq t}">selected="selected" </c:if> value="${t}">${t}</option>
	                    	</c:forTokens>
                        </select>
                    </td>
                </tr>
             ...
             <%--   <tr>
                ...
                <tr style="height:40px">
                    <td colspan="2" style="text-align: center">
                        ...
                        <input type="button" class="btn btn-danger" id="endTask" value="结束">&nbsp;&nbsp;
                        <input type="button" class="btn btn-success" onclick="top.closeWindow('请假申请详情')" value="取消">
                    </td>
                </tr>
                </tbody>
            <%--薄荷、百里香、迷迭香、碰碰香、虎尾兰、柠檬香蜂草、白鹤芋、薰衣草、茉莉、栀子花、铜钱草、芦荟--%>
            </table>
...
<script>
	...
	$(function(){
		...
		$("#save").click(function(){
			$.ajax({
				  url: "/workProcessApply/relevantHandleTask",
				  dataType: "json",
				  type:"post",
				  async: false,
				  data: $("#formId").serialize(),
