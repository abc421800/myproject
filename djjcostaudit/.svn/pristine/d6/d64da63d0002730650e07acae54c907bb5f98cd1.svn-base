  ...
  <body>
    ...
    <div data-options="region:'center',border:0" class="wrap_center">
		<form method="post" id="form">
			<div class="tabox table-responsive float-table">
				<input type="hidden" id="currentId" name="id" value="${c.id}" />
				<table class="table table-bordered table-condensed">
					<tr>
						<th class="text-right">角色名称</th>
						<td class="text-left">
						<span>
							<input name="name" id="name" value="${c.name }" type="text" class="input-sm form-control" />
						</span></td>
					</tr>
					<tr>
						...
						<td class="text-left">
							<span>
								<input name="num" id="num" value="${c.num}" type="text" class="input-sm form-control" />
							</span>
						</td>
					</tr>
					<tr>
						...
						<td class="text-left">
							<span>
								<textarea name="description"  rows="5" cols="70" class="form-control">${c.description}</textarea>
							</span>
						</td>
					</tr>
				</table>
    ...
    $(function(){
		$("#save").click(function(){
			...
			$.ajax({
				  url: "/projectStakeholderroler/save",
				  dataType: "json",
				  type:"post",
				  async: false,
				  data: $("#form").serialize(),
				  success: function(result){
					  if(result.status==200){
						  var json1={tabTitle:'项目干系人',url:'/forward_projectStakeholderrole_list'};
