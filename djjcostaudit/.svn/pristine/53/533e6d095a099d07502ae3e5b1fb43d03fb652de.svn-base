    ...
    <div class="filter">
        <div class="form-inline">
            <div class="form-group">
                ...
                <input type="text" id="projectName" placeholder="请输入关键字" class="form-control input-sm">
            </div>
            <div class="form-group">
                <input type="submit"  value="查询" class="btn btn-primary" onclick="$('#name').val('');queryProject()">&nbsp;
            	<input type="reset"  value="重置" class="btn btn btn-success" onclick="$('#projectName').val('');$('#code').val('');queryProject();">&nbsp;
            </div>
        </div>
    </div>
</div>
        ...
        columns: [
            [
            {
                ...
                formatter: function (value, rowData, index) {
                	if(value!=null){
                        return "<span title="+value+" style=\"color:#549de3;padding-left:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block;\" onclick=\"showInfoPro('"+rowData.id+"')\">"+value+"</span>";
                	}else{
                		return ""
                	}
                }
