...
<div class="row">
    <div class="filter">
        <div class="form-inline">
            ...
            <div class="form-group">
                <label for="">审定资料单价共享：</label>
                <select id="category" onchange="searchTask();" class="form-control input-sm" style="width: 100px">
                    <option value="">请选择</option>
...
<div class="row">
    ...
    <shiro:hasPermission name="task:add">
        <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" plain="false" onclick="add('djsh');">添加</a>
    </shiro:hasPermission>
    <%-- 
...
<script src="${pageContext.request.contextPath}/res/js/cost/task/priceCheck.js"></script>
<script>
...
function exportDate(){
	var selections = $('#priceCheck').datagrid('getSelections');
   	...
   	window.location.href = "${path}/costTask/exportTaskDj?ids="+ids;
   }
$(function(){
	 $.post("/costTaskType/listByName", {"name":"单价审核"},function(result){
		 for(var i in result){
			var v=result[i].name;
	 ...
	 //导入
function importTask(){
	var url = "/forward_task_importTaskDj";
	$("#importTask").dialog({
	title: "选择文件",
	iconCls:'icon-info',
    minimizable:false,
