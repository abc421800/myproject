        ...
        <div class="slider-db" style="margin-top: 10px;">
			...
			<div class="row">
				<form method="post" id="form">
					...
					<shiro:hasPermission name="taskType:update">
					<div class="row">
...
function loadForm(id){
	...
	$('.slider-db').show();
	if(id){
		var url = "/costTaskType/findDetails.do?id="+id;
		$("#form").form("load",url);
	}else{  
		//请空表单的数据
		$("#form").form("clear");
...
//加载子列表
  function loadChildren(parentId){
     ...
     fitColumns:true,
	   ...
	   pageList:[10,20,30],
	   columns:[[   
			 {field:'name',title:'审价类型名称',width:150,align:'center'},
      ...
      toolbar: [
       <shiro:hasPermission name="taskType:add">
           { 
           text: '添加', 
           iconCls: 'icon-add', 
           handler: function() { 
       ...
       <shiro:hasPermission name="taskType:delete">
       {
           text: '删除', 
           iconCls: 'icon-remove', 
           handler: function() {
          		var ids="";
	            	...
	            	$.messager.confirm('提示','确定删除？',function(r){
	            		if(r){
			            	...
			            	$.post("/costTaskType/del", {"ids":ids},function(data){
			            		if(data.status == 200){
	    ...
	    onBeforeLoad:function(param){
	    	$.post("/costTaskType/list?parentId="+parentId,param,function(result){
	    		  if(result){
