...
public class WorkPersonController {
	...
	public  Float  getDaysBetween(Date startDate, Date endDate) {  
        ...
        return (float) ((toCalendar.getTime().getTime() - fromCalendar.getTime().getTime()) / (1000 * 60 * 60 * 24)+1);  
    }  
	...
	public String toEdit(HttpServletRequest request){
		...
		if(null!=id && !"".equals(id)) {
			...
			String actualStartTimeStr=workPerson.getActualStartTimeStr();
			String actualEndTimeStr=workPerson.getActualEndTimeStr();
			SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
			try {
				if("有效"==workPerson.getEffectiveFlag()){
						Float days = getDaysBetween(sdf.parse(actualStartTimeStr),new Date());
						if(days<0){
							workPerson.setTotalDay(0.0f);
						}else{
							workPerson.setTotalDay(days);
						}
				}else{
					if(null!=actualEndTimeStr){
						Float days = getDaysBetween(sdf.parse(actualStartTimeStr),sdf.parse(actualEndTimeStr));
						workPerson.setTotalDay(days);
					}else{
						Float days = getDaysBetween(sdf.parse(actualStartTimeStr),new Date());
						if(days<0){
	...
	public GlobalResult save(HttpServletRequest request,WorkPerson workPerson){
		...
		try {
			...
			if(null==wp){
				...
				workPerson.getActualStartTimeStr();
				...
				//插入当月的登记表
				Date actualStartTime= workPerson.getActualStartTime();
