...
public class CostContractController {
    ...
    public EUDataGridResult list(HttpServletRequest request, CostContract costContract, int page, int rows) {
        ...
        try {
            ...
            CostEnterprise ent = null;
            if (null != enterpriseCode && !"".equals(enterpriseCode)) {
                ent = costEnterpriseService.selectByCostEnterpriseCode(enterpriseCode);
            }
            if (null != ent) {
                map.put("partyB", ent.getSimpleName());
            }
            //摇号关联合同****
...
//                }
    ...
    @ResponseBody
    public void addTask() {
        List<CostContract> contractList = costContractService.selectTask();
        for (CostContract contract : contractList) {
            CostTask costTask = new CostTask();
            costTask.setId(OrderUtil.buildOrderId(""));
            costTask.setProjectId(contract.getProjectId());
            costTask.setProjectName(contract.getProjectName());
            costTask.setContractId(contract.getId());
            costTask.setContractName(contract.getName());
            costTask.setCode(contract.getCode());
            costTask.setDeliveryFlag("否");
            costTask.setCreater(contract.getCreater());
            costTask.setStatus("审结");
