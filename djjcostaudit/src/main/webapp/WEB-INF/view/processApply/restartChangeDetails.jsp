...
<div data-options="region:'center',border:0" class="wrap_center">
    <form action="" id="formId">
    	...
    	<table class="table_edit" id="table" style="margin-bottom: 10px;" cellspacing="0" cellpadding="0" >
                ...
                <tr style="display: none;" id="remarkBz1">
                    ...
                    <td>
                        <select name="remarkQt" class="form-control input-sm" style="width:98%;display: inline-block"  id="remarkQt">
                            <c:forTokens items="回公司,公司会议,其他" delims="," var="i">
                                <option <c:if test="${i eq obj.remark }">selected = "selected"</c:if>  value="${i}">${i}</option>
                            </c:forTokens>
                        </select>
                    </td>
                </tr>
                <tr style="display: none;" id="remarkBz2">
                    <td class="bgcolor"><strong>*</strong><span id="remarkStar">备注：</span></td>
                    <td>
                ...
                <tr>
                    <td class="bgcolor"><strong>*</strong>变更原因：</td>
                    <td>
                ...
                <tr style="height:40px">
                    <td colspan="2" style="text-align: center">
                        <input type="button" class="btn btn-primary" id="save" value="确认变更">&nbsp;&nbsp;
                        <input type="button" class="btn btn-success" onclick="top.closeWindow('请假申请变更')" value="取消">
                    </td>
                </tr>
                </tbody>
            </table>
...
<script>
    var annualLeaveStartTimeStr="${workPerson.annualLeaveStartTimeStr}";
    var annualLeaveEndTimeStr="${workPerson.annualLeaveEndTimeStr}";
    <!-- 禁用所有Form表单 -->
	...
	$(function(){
		...
		$("#save").click(function(){
            ...
            if(opinion==""){
                $.messager.alert({title:'温馨提示', msg:'请填写变更原因！', icon: 'error', top:100});
                return;
            }
            if(type=="其他"){
            ...
            //判断年假范围
            if(type=="年休假"){
                var leaveTimeStart=leaveTime.substring(0,10);
                var leaveTimeEnd=leaveTime.substring(leaveTime.length-10, leaveTime.length);
                if(leaveTimeStart<annualLeaveStartTimeStr || leaveTimeStart>annualLeaveEndTimeStr){
                    $.messager.alert({title:'温馨提示', msg:'您的请假时间不在可休年假范围内,不可请年休假！', icon: 'error', top:100});
                    return;
                }
            }
			$.ajax({
				  url: "/workProcessApply/Change",
				  dataType: "json",
				  type:"post",
				  async: false,
