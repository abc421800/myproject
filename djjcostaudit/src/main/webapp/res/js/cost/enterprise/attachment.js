...
function saveFileName(){
	...
	var rows = $("#material").treegrid("getSelections");
	var json1={};
	if(rows.length== 0){
		json1.name=projName;
		json1.typeId=rukuEnId;
	}else{
		json1.name=projName;
		json1.typeId=rukuEnId;
		json1.pid=rows[0].id;
		...
		if(r){
        	...
        	$.post("/costAttachment/del", {"ids":ids},function(result){
        		if(result.status==200){
        		  ...
        		  window.parent.showMessager("温馨提示", "<b>操作成功!</b>", 5000, "slide");
  			      $("#material").treegrid('reload');
  			      $('#material').treegrid('clearSelections');
  			    }else{
  				  $.messager.alert("温馨提示","操作异常,请联系管理员!", "error");
  			    }
        	}, "json");
...
function uploadFile(){
    ...
    var url = "/forward_document_fileinput?folderId="+folderId+"&typeId="+rukuEnId;
    $("#dialog").dialog({
        title: "选择文件",
        minimizable:false,
