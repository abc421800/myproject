...
function leaveRequestCreat() {
    ...
    $('#leaveRequestCreat').datagrid({
        url: '/workProcessApply/list?flag=task',
        loadMsg: '数据加载中,请稍候...',
        nowrap: false,
        rownumbers: true,
        height: tableHeight(),
        ...
        frozenColumns:[[
			{
			    ...
			    formatter: function (value, rowData, index) {
			    	if(value){
                    	if(value=='结束'){
                    		return "<div class='btn-success'> "+value+"</div>";
                    	}else if(value=='驳回'){
                    		return "<div class='btn-danger'> "+value+"</div>";
                    	}else if(value=='审批中'){
                    		if(currentUser==rowData.nextperson){
                    			return "<div class='btn-info'>待办</div>";
                    		}else{
                    			return "<div class='btn-info'> "+value+"</div>";
                    		}
                    	}
                    }else{
                        return ""
            ...
            [
               {
                    ...
                    sortable: false,
                    formatter: function (value, rowData, index) {
                    	var start=rowData.leaveStartTimeStr;
                    	var end =rowData.leaveEndTimeStr;
                    	if(start && end){
                    		return getDays(rowData.leaveStartTimeStr,rowData.leaveEndTimeStr);
                    	}else{
                    		return "";
                    	}
                    }
        ...
        onLoadSuccess: function (data) {
            if (data.total == 0) {
                $('#leaveRequestCreat').datagrid('loadData', {total: 1, rows: [{proJob: "未找到相关信息！"}]});
            }
        },
